...
function bypass_shortlinks($url, $separator = 0) {
    ...
    if (preg_match("#(luckydice.net|kalimbanote.com|cryptoflare.cc|myhealths.icu|clk.st|urlsfly.me|wefly.me|shortsfly.me|linksfly.me|clicksfly.me|3link.co)#is", $host)) {
        ...
        if (preg_match("#(link1s.com)#is", $host)) {
            ...
            $referer = "https://infonerd.org/";
        } elseif (preg_match("#(845265.xyz)#is", $host)) {
            $referer = "https://reintroducing.us/";
        } elseif (preg_match("#(151989.xyz)#is", $host)) {
            $referer = "https://writeprofit.org/";
            ...
            $referer = "https://globalrecipes.us/";
        } else {
            $referer = 0;
        }
        ...
        if (preg_match("#(go.bitss.sbs|go.shtfly.com|revcut.net|l2.revcut.net|go.urlcut.pro|go.faho.us|go.eazyurl.xyz|link.eazyurl.xyz|go.cutlink.xyz|bitcosite.com|m.wdu.info|m.addurl.biz)#is", $host)) {
            $r = base_short($url_ori, 0, 0, 0, $cloud, 0, 0, $proxy);
            if (!$r["url1"][0]) {
                return "refresh";
            }
            $host_ref = parse_url($r["url1"][0])["host"];
            #die(print_r($r));
            $referer =  "https://".$host_ref."?safelink_redirect=".base64_encode(json_encode(["second_safelink_url" => "", "safelink" => $url]));
           L(200);
        } 
