...
function bypass_shortlinks($url, $separator = 0) {
    global $only_sl, $request_proxy, $method_proxy;
    ulang:
    ...
    if (preg_match("#(luckydice.net|kalimbanote.com|cryptoflare.cc|myhealths.icu|clk.st|urlsfly.me|wefly.me|shortsfly.me|linksfly.me|clicksfly.me|3link.co)#is", $host)) {
        ...
        if (explode('"', $t[1][2])[0] == "ad_form_data") {
            ...
            $r1 = base_short(str_replace("pwrpa.cc", "pwrpa.cc/go", str_replace("jameeltips.us", "jameeltips.us/blog", str_replace("techyuth.xyz", "techyuth.xyz/blog", $run["go"][0]))), 1, $data, 0, $cloud, $cookie)["json"];
            #die(print_r($r1));
        ...
        if (explode('"', $t[1][7])[0] == "country") {
            ...
            $r1 = base_short( $run["go"][0], 1, $data, 0, $cloud, $cookie)["json"];
        ...
        $r1 = base_short($run["links"],"", $data, 0, $cookie);
        ...
        if ($r1["timer"] or $r1["timer"] == 0) {
            ...
            $r2 = base_short($run["go"][0], 1, $data, 0, 0, $cookie)["json"];
      ...
      if (strpos($link, "http") !== false) {
    ...
    } elseif (preg_match("#(ctr.sh|easycut.io|revcut.net|crypto-radio.eu|todaynewsview.store|shrinkme.link|faho.us|urlcut.pro|ez4short.com|bitad.org|cutlink.xyz|bitss.sbs|inlinks.online|shortino.link|sharecut.io|droplink.co|adbitfly.com|earnify.pro|btcut.io|slfly.net|info.linkzfly.xyz)#is", $host)) {
        ...
        $cookie[] = $r["cookie"];
        $r = base_short($url1["url"]."?url8j=".$url, 0, 0, $url0, 0, $cookie);
        $cookie[] = $r["cookie"];
