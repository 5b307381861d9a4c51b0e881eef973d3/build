...
function bypass_shortlinks($url, $separator = 0) {
    ...
    if (preg_match("#(luckydice.net|kalimbanote.com|cryptoflare.cc|myhealths.icu|clk.st|urlsfly.me|wefly.me|shortsfly.me|linksfly.me|clicksfly.me|3link.co)#is", $host)) {
        ...
        if (preg_match("#(link1s.com)#is", $host)) {
        ...
        } elseif (preg_match("#(shrinke.us|shrinkme.info|shrinkme.org|shrinkme.us|shrinkme.site|shrinkme.cc|shrinkme.vip)#is", $host)) {
            ...
            $referer = "https://coingraph.us?safelink_redirect=eyJzZWNvbmRfc2FmZWxpbmtfdXJsIjoiIiwic2FmZWxpbmsiOiJodHRwczpcL1wvcmV2Y3V0Lm5ldFwvZkRzTSJ9";
        ...
        } elseif (preg_match("#(loptelink.com)#is", $host)) {
            $referer = "https://www.gamelopte.com/";
        } elseif (preg_match("#(link.tfly.link)#is", $host)) {
            $referer = "https://bioinflu.com/";
        } elseif (preg_match("#(linx.cc)#is", $host)) {
            $referer = "https://autodemi.com/";
        } else {
            $referer = 0;
        }
        ...
        if (preg_match("#(go.bitss.sbs|489651.xyz|546512.xyz|go.shtfly.com|revcut.net|l2.revcut.net|121989.xyz|go.urlcut.pro|131989.xyz|go.faho.us|141989.xyz|go.eazyurl.xyz|link.eazyurl.xyz|go.cutlink.xyz|151989.xyz|120898.xyz|bitcosite.com|161989.xyz|845265.xyz|m.wdu.info|m.addurl.biz)#is", $host)) {
            ...
            if ($alt_reff) {
              ...
              $referer = "https://away.vk";
            }
        }
        $run = build($url);#die(print_r($referer));
        #$r = base_short($url_ori, 0, 0, $refeurer, $cloud, 0, 0, $proxy);
        ...
        if (explode('"', $t[2][2])[0] == "captcha") {
            ...
            if (preg_match("#(link.tfly.link)#is", $host)) {
                $method = "hcaptcha";
                $type = "h-recaptcha-response";
            } elseif (!$r[$method]) {
    ...
    } elseif (preg_match("#(_tii.la|_tei.ai|_lnbz.la)#is", $host)) {
...
#print_r($r1);
      ...
      if (strpos($link, "http") !== false) {
        ...
        #die(print_r($r));
        $cookie[] = $r["cookie"];
        $url0 = urldecode($r["url6"]) ?: urldecode($r["url"]);
      ...
      if ($r["url"]) {
        ...
        if ($t[1][0] == "my_key") {
            ...
            if ($r1["status"] == 200) {
                ...
                if ($r["go_link"]) {
                    $link = json_encode(["status" => "success", "url" => $r["go_link"]]);
                    if ($only_sl) {
